project.Behavior.fixFormPriceFields = function (context) {
	...
	if ($priceFieldsContainer.length) {
		function disableCheckboxes(force) {
...
project.Behavior.scrollToForm = function (context) {
	if (window.location && window.location.hash === '#scroll-to-form' && $('.js-scroll-form').length) {
...
project.Behavior.menuFixed = function (context) {
	...
	var $nav = $(".main-header");
	if ($('body').hasClass('page-template-template-why-new-apartments')) {
		return;
	}
