...
project.Behavior.setEqualHeightBlocks = function (context) {
    var $el = $('.js-equal-height, .project-item', context);
    if ($el.length) {
        $el.matchHeight({
            byRow: true,
            property: 'height',
...
project.Behavior.mobMenu = function (context) {
    ...
    if ($toggleMenuItems.length) {
        $toggleMenuItems.on('click', function (event) {
...
/**
 * init address autocomplete via ajax
 * @param context
 */
project.Behavior.initAddressAutocomplete = function (context) {
    var $input = $('.js-autocomplete-address', context);
    if ($input.length) {
        $input.autocomplete({
            source: function (request, response) {
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        'action': '_getAllCities',
                    },
                    success: function (data) {
                        if(data.success != data.error && data.cities) {
                            var filteredCities = [];
                            $.each(data.cities, function(item, value) {
